<script lang="ts">
	...
	export async function execute(crsKind: string, crsType: string, material: string) {
		...
		stiffnessDataToShow = [
			...
			{
				id: 'EI',
				val: capacityResponse.EI ? capacityResponse.EI.toExponential(2) : 'NaN',
				unit: 'Nmm^2'
			},
			...
			{
				id: 'EI_y',
				val: capacityResponse.EI_y ? capacityResponse.EI_y.toExponential(2) : 'NaN',
				unit: 'Nmm^2'
			},
			{
				id: 'EI_z',
				val: capacityResponse.EI_y ? capacityResponse.EI_z.toExponential(2) : 'NaN',
				unit: 'Nmm^2'
			}
		];
		CrsClassDataToShow = [
		...
		capacityDataToShow = [
			// All sections
			{
				id: 'N_{pl}',
				val: (
					(icludeSafetyFactor ? capacityResponse.N_pl_d : capacityResponse.N_pl_k) / 1000
				).toFixed(0),
				unit: 'kN'
			},
			...
			// Symmetric sections
			{
				id: 'V_{pl}',
				val: (
					(icludeSafetyFactor ? capacityResponse.V_pl_d : capacityResponse.V_pl_k) / 1000
				).toFixed(0),
				unit: 'kN'
			},
			{
				id: 'M_{el}',
				val: (
					(icludeSafetyFactor ? capacityResponse.M_el_d : capacityResponse.M_el_k) / 1000000
				).toFixed(0),
				unit: 'kNm'
			},
			{
				id: 'M_{pl}',
				val: (
					(icludeSafetyFactor ? capacityResponse.M_pl_d : capacityResponse.M_pl_k) / 1000000
				).toFixed(0),
				unit: 'kNm'
			},
			...
			// Assymetric sections
			{
				id: 'V_{pl,y}',
				val: (
					(icludeSafetyFactor ? capacityResponse.V_pl_y_d : capacityResponse.V_pl_y_k) / 1000
				).toFixed(0),
				unit: 'kN'
			},
			{
				id: 'V_{pl,z}',
				val: (
					(icludeSafetyFactor ? capacityResponse.V_pl_z_d : capacityResponse.V_pl_z_k) / 1000
				).toFixed(0),
				unit: 'kN'
			},
			{
				id: 'M_{el,y}',
				val: (
					(icludeSafetyFactor ? capacityResponse.M_el_y_d : capacityResponse.M_el_y_k) / 1000000
				).toFixed(0),
				unit: 'kNm'
			},
			{
				id: 'M_{pl,y}',
				val: (
					(icludeSafetyFactor ? capacityResponse.M_pl_y_d : capacityResponse.M_pl_y_k) / 1000000
				).toFixed(0),
				unit: 'kNm'
			},
			{
				id: 'M_{el,z}',
				val: (
					(icludeSafetyFactor ? capacityResponse.M_el_z_d : capacityResponse.M_el_z_k) / 1000000
				).toFixed(0),
				unit: 'kNm'
			},
			{
				id: 'M_{pl,z}',
				val: (
					(icludeSafetyFactor ? capacityResponse.M_pl_z_d : capacityResponse.M_pl_z_k) / 1000000
				).toFixed(0),
				unit: 'kNm'
			}
		];
